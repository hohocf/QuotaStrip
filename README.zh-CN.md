...
点击菜单栏图标，可独立勾选「显示 CC（Claude Code）」和「显示 Codex」，选择会在重启后保留。取消勾选会隐藏对应 Touch Bar 面板并停止该服务的数据刷新；「立即刷新」也只刷新已勾选的服务。两项都取消时不启动数据读取进程。切换时会终止正在运行的读取进程，但已经发出的请求无法撤回。

Codex 的额度与重置时间完全读取本地 `~/.codex/sessions` 会话日志，不访问网络，也不读取账号凭据；「立即刷新」同样如此。本地日志未更新时无法获得最新服务器额度。CC 启用时沿用现有用量 API 和缓存机制。

Codex 面板按日志中的 `window_minutes` 显示实际额度周期，不把主/次窗口固定解释为 5h/7d。只有周额度时居中显示一行 7d；缺失窗口不显示，已过期或未知的用量显示「—」，等待本地日志更新。

菜单栏自动显示本地日志 `plan_type` 对应的 Codex 账号类型（例如 Plus、Pro 或 Pro (prolite)），无需手动切换。账号类型仅用于提示；实际显示几行仍由日志中的额度窗口决定，因此 Plus 和 Pro 都可显示 5h/7d 两行，也支持只有周额度的账号。换号后会随新的本地额度记录更新；未写入新记录前仍可能显示旧账号信息。

### 版本信息

在菜单栏点击「关于 QuotaStrip…」，可查看版本号、构建号和 GitHub 项目链接。当前版本：**1.1.1（构建 3）**。更新内容见 [CHANGELOG.md](CHANGELOG.md)。

仅勾选 Codex 时，面板加宽，在重置倒计时右侧显示具体重置日期、24 小时时间和英文星期缩写（如 Sat，采用系统时区）；同时勾选 CC 和 Codex 时保留原来的紧凑布局。

时间格式统一为原来的 24 小时制 `15:30`，包括紧凑布局的 5h 重置时间；完整日期示例：`2026-09-19 15:30 Sat`。
